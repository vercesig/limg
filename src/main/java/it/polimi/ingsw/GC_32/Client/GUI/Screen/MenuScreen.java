...
public class MenuScreen extends BorderPane{
	...
	public void setEvent(){
		...
		this.buttons.forEach(button ->{
			...
			case 1:
				this.buttons.get(1).getChildren().get(0).setOnMouseClicked(click->{ this.game.getBoard().changeToTower();
				GameUtils.update(this.game);});
				break;
			
			case 2:
				this.buttons.get(2).getChildren().get(0).setOnMouseClicked(click->{ this.game.getBoard().changeToBoard();
																					GameUtils.update(this.game);});
				break;
				
			case 3:
				this.buttons.get(3).getChildren().get(0).setOnMouseClicked(click-> {this.game.getBoard().changeToPersonal();
																					GameUtils.update(this.game);});
				break;
				
			case 4:
				this.buttons.get(4).getChildren().get(0).setOnMouseClicked(click -> {this.game.getExtraScreen().setProfile();
																					GameUtils.update(this.game);});
				break;
				
			case 5:
				this.buttons.get(5).getChildren().get(0).setOnMouseClicked(click-> {this.game.getExtraScreen().setChat();
																					GameUtils.update(this.game);});
				break;
				
			case 6:
				this.buttons.get(6).getChildren().get(0).setOnMouseClicked(click-> {
